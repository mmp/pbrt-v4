...
namespace pbrt
{
  ...
  //NOTE: Just putting cap on it now, so it doesn't go crazy
  constexpr int MAX_OUTPUT_REMAINING = 1000;

  using namespace pbrt;
  class SimpleVolPathIntegratorNN : public RayIntegrator
  {
  public:
    SimpleVolPathIntegratorNN(int maxDepth, Camera camera, Sampler sampler,
      Primitive aggregate, std::vector<Light> lights
    );
    ...
    static std::unique_ptr<SimpleVolPathIntegratorNN> Create(
      const ParameterDictionary& parameters, Camera camera, Sampler sampler,
      Primitive aggregate, std::vector<Light> lights, const FileLoc* loc
    );
  ...
  private:
    ...
    /// @brief Logs formatted string to output log file
    /// @tparam ...Args 
    /// @param fmt 
    /// @param ...args 
    /// @return number of lines available left
    template <typename... Args>
    inline int LogToFile(std::ofstream& stream, const char* fmt, Args &&...args) const
    {
      ...
      if (stream.write(ret.c_str(), ret.length()))
      {
        // remainingLines -= std::count(ret.begin(), ret.end(), '\n');
      }
    ...
    int maxDepth;
    mutable int remainingLines = MAX_OUTPUT_REMAINING;
    mutable std::ofstream m_OutputFS;
    mutable std::ofstream m_InputFS;
