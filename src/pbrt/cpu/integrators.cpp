...
namespace pbrt
{
    ...
    SimpleVolPathIntegratorNN::SimpleVolPathIntegratorNN(
        int maxDepth, Camera camera, Sampler sampler,
        Primitive aggregate, std::vector<Light> lights
    )
        : RayIntegrator(camera, sampler, aggregate, lights), maxDepth(maxDepth)
    {
        // m_OutputLogFileName = DEFAULT_PRIMITIVE_OUTPUT_FILE;

        m_InputFS.open(DEFAULT_PRIMITIVE_INPUT_FILE, std::ios::binary | std::ios::ate);
        m_OutputFS.open(DEFAULT_PRIMITIVE_OUTPUT_FILE, std::ios::binary | std::ios::ate);
    }
    ...
    //TODO: Properly consider exactly what kind of information I want to output, after single bounce? k-bounces? only surface outputs?
    //NOTE: CONSIDER OUTPUTTING THE INPUT RAY INFORMATION LIKE POSITION, DIRECTOIN AND OTHER MATERIAL PROPERTIES INTO 
    // INSTANT NGP TO DO MULTIRESOLUTION HASHING, AND THEN INPUT THAT INTO A MLP THAT ATTEMPTS TO SIMULATE AT 1 BOUNCE/MAX DEPTH
    // ESSENTIALLY, CAN WE ABSTRACT THE SAMPLING ALONG THE RAY TO A SINGLE LOOKUP THAT WILL HOLD THE FINAL RADIANCE/RGB VALUE??
    // LIKE INSTEAD OF IN NERF INPUTTING THE 5D SPATIO-DIRECTIONAL INFORMATION + OTHER MATERIAL INFORMATION, WE INPUT RAY INFORMATION?
    // AND DO THE SINGLE LOOK UP INSTEAD? NEED TO FIRST TEST WITH NO BOUNCES/SINGLE BOUNCE TO SEE THE OUTPUT AND DO IT FIRST FOR THE SIMPLEVOLPA6THINTEGRATOR

    SampledSpectrum SimpleVolPathIntegratorNN::Li(
        RayDifferential ray, SampledWavelengths& lambda, Sampler sampler, ScratchBuffer& buf, VisibleSurface* visibleSurface) const

    {
        ...
        while (true)
        {
            ...
            if (ray.medium)
            {
                ...
                auto updateRayState = [&](Point3f p, MediumProperties mp, SampledSpectrum sigma_maj, SampledSpectrum T_maj)
                    {
                        // Find probabilities for each of the scattering events for only the first wavelength sample.
                        Float pAbsorb = mp.sigma_a[0] / sigma_maj[0];
                        Float pScatter = mp.sigma_s[0] / sigma_maj[0];
                        Float pNull = std::max<Float>(0.0f, 1 - pAbsorb - pScatter);

                        int mode = SampleDiscrete({ pAbsorb, pScatter, pNull }, uMode);
                        constexpr int ABSORB = 0;
                        constexpr int SCATTER = 1;
                        constexpr int NULL_EVENT = 2;
                        if (mode == ABSORB)
                        {
                            // Absorption event by participating media, terminate ray
                            terminated = true;

                            // Accumulate media's emission radiance into Li
                            L += beta * mp.Le;

                            // Ray terminated, end sampling
                            return false;
                        }
                        else if (mode == SCATTER)
                        {
                            // Out Scattering event, update beta and ray state. 
                            // If maxDepth reached, terminate ray
                            if (depth++ >= maxDepth)
                            {
                                terminated = true;
                                return false;
                            ...
                            // Sample new 3D angles theta and phi
                            Point2f u{ rng.Uniform<Float>(), rng.Uniform<Float>() };
                            pstd::optional<PhaseFunctionSample> ps = mp.phase.Sample_p(-ray.d, u);


                            // Phase function did not return valid sample
                            // TODO: Figure out when/why this could happen
                            if (!ps)
                            {
                                terminated = true;
                                return false;
                            ...
                            // Update beta according to newly sampled direction in phase function
                            //TODO: Figure out why to do this/what this division represents and what p and pdf represent
                            beta *= (ps->p / ps->pdf);
                            ray.o = p;
                            ray.d = ps->wi;
                            scattered = true;

                            // No more sampling along this ray, we have scattered so return false
                            return false;

                        }
                        else if (mode == NULL_EVENT)
                        {
                            // Null scattering event, we ignore and just continue but update in case we sample
                            // new point again so we don't reuse uMode this randomness
                            uMode = rng.Uniform<Float>();
                            return true;
                        }
                        else
                        {
                            ErrorExit("Invalid volumetric event of %d, exiting...\n", mode);
                        }

                    };

                // Sample medium along ray using delta tracking to get transmission radiance
                SampleT_maj(ray, tMax, u, rng, lambda, updateRayState);
            }

            // If terminated due to absorption or max depth, return accumulated Li
            if (terminated)
            {
                if (remainingLines > 0)
                {
                    --remainingLines;
                    LogToFile(m_InputFS, "%s\n", ray.ToString());
                    LogToFile(m_OutputFS, "[ Ray %s's accumulated radiance transmission is: %s ] \n", ray.ToString(), L.ToString());
                }
                return L;
            }

            // If scattered, continue loop with new ray
            if (scattered)
            {
                continue;
            }

            // If neither terminated nor scattered, accumulate radiance from emitters interacted with (lights or surface interaction)
            if (si)
            {
                // Surface interaction, accumulate emission from that surface
                L += beta * si->intr.Le(-ray.d, lambda);
            }
            else
            {
            ...
            // Need to handle if surface has BSDF (aka reflectance or scattering)
            // If has valid BSDF, error
            BSDF bsdf = si->intr.GetBSDF(ray, lambda, camera, buf, sampler);
            if (!bsdf)
            {
                // No BSDF, should be a surface with media boundary, skip intersection to move ray into next volume
                si->intr.SkipIntersection(&ray, si->tHit);
            }
            else
            {
                // Need to check if this is a 0 value BSDF, if not we error
                Float uc = sampler.Get1D();
                Point2f u = sampler.Get2D();
                if (bsdf.Sample_f(-ray.d, uc, u))
                {
            ...
            // Log(LogLevel::Verbose, DEFAULT_PRIMITIVE_INPUT_FILE.c_str(), remainingLines,
            //     "%s", ray.ToString()
            // );

            // Log(LogLevel::Verbose, DEFAULT_PRIMITIVE_OUTPUT_FILE.c_str(), remainingLines,
            //     "[ Ray %s's accumulated radiance transmission is: %s ] ", ray.ToString(), L.ToString()
            // );
        }
        if (remainingLines > 0)
        {
    ...
    std::string SimpleVolPathIntegratorNN::ToString() const
    {
        return StringPrintf("[ SimpleVolePathIntegratorNN maxDepth: %d ] ", maxDepth);
    }

    std::unique_ptr<SimpleVolPathIntegratorNN> SimpleVolPathIntegratorNN::Create(
        const ParameterDictionary& parameters, Camera camera, Sampler sampler,
        Primitive aggregate, std::vector<Light> lights, const FileLoc* loc)
    {
        int maxDepth = parameters.GetOneInt("maxdepth", 5);
        return std::make_unique<SimpleVolPathIntegratorNN>(maxDepth, camera, sampler, aggregate,
            lights);
    }
        ...
        const RGBColorSpace* colorSpace, const FileLoc* loc)
    {
        ...
        else if (name == "simplevolpathnn")
            integrator = SimpleVolPathIntegratorNN::Create(parameters, camera, sampler,
                aggregate, lights, loc);
