...
//NOTE: For now to deal with multiple images, just add a count;
static int imagesWritten = 0;
...
namespace pbrt
{
    ...
    bool Image::Write(std::string name, const ImageMetadata& metadata) const
    {
        ...
        if (imagesWritten > 0)
        {
            std::string baseFilename = RemoveExtension(name);

            //TODO: Improve to support different filename extensions
            name = StringPrintf("%s_%d%s", baseFilename, imagesWritten, ".exr");
            imagesWritten++;
        }
        if (HasExtension(name, "exr"))
